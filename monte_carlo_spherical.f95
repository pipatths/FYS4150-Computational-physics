...
DOUBLE PRECISION FUNCTION int_function(x)
  ...
  REAL(DP) :: exp1, exp2, deno
    ...
    deno = SQRT(x(1)**2 + x(2)**2 - 2*x(1)*x(2)* &
           (COS(x(3))*COS(x(4)) + SIN(x(3))* &
           SIN(x(4))*COS(x(5)-x(6))))

    int_function = x(1)**2*x(2)**2*SIN(x(3))*sin(x(4))/deno
...
PROGRAM integral
  ...
  int_MC = 0.
  int_MC2 = 0.
  jacobi_det = 4*pi**4*(1./16)

  OPEN(UNIT=5,FILE='improved_MC_mpi.dat')   
  DO c = 1, 5
